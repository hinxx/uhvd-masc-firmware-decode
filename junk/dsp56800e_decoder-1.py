...
def find_frames(data: bytes) -> list[dict]:
    """Locate every frame in `data`.

    A frame begins at a codeword-aligned (4-byte) offset whose first
    4 bytes match a valid sync and whose next 4 bytes have a valid
    tag suffix (bytes 2-3 ∈ {(0x26,0x9F),(0xBF,0x53)}).

    Returns frames in file order; each frame's `frame_size` is the
    distance to the next sync (or end of file).
    """
    sync_positions = []
    for pos in range(0, len(data) - FRAMING + 1, 4):
        if bytes(data[pos:pos + SYNC_LEN]) not in VALID_SYNCS:
            continue
    ...
    if not sync_positions:
        raise ValueError("no valid sync words found in file")

    frames = []
    for i, start in enumerate(sync_positions):
        end = sync_positions[i + 1] if i + 1 < len(sync_positions) else len(data)
        size = end - start
