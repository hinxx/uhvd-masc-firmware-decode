...
def _decode_codeword_detail(codeword: bytes,
                            mappings: Sequence[dict],
                            nibble_order: tuple[int, int, int, int],
                            byte_order: str,
                            ) -> dict:
    """Return display fields for one 4-byte codeword."""
    pids = tuple(PAIR_INDEX[p][codeword[p]] for p in range(4))
    nibbles = tuple(mappings[p][pids[p]] for p in range(4))
    shifts = tuple(12 - 4 * nibble_order[p] for p in range(4))
    word = sum(nibbles[p] << shifts[p] for p in range(4))
    packer = ">H" if byte_order == "big" else "<H"
    return {
        "pair_ids": pids,
        "nibbles": nibbles,
        "shifts": shifts,
        "word": word,
        "output": struct.pack(packer, word),
    }


def _trace_line(file_off: int,
                ...
                byte_order: str,
                note: str,
                output_cw: int | None = None,
                ) -> None:
    prefix = f"0x{file_off:08X}"
    if frame_index is None:
        frame = "frame=-"
    else:
        frame = f"frame={frame_index:04d}"
    rel = "rel=-" if rel_off is None else f"rel=0x{rel_off:04X}"
    cw = "-" if output_cw is None else str(output_cw)

    try:
        detail = _decode_codeword_detail(
            codeword, mappings, nibble_order, byte_order)
        pids = ",".join(f"{pid:X}" for pid in detail["pair_ids"])
        nibs = ",".join(f"{nb:X}" for nb in detail["nibbles"])
        shifts = ",".join(str(s) for s in detail["shifts"])
        converted = (f"pairs=({pids}) nibbles=({nibs}) shifts=({shifts}) "
                     f"word=0x{detail['word']:04X} "
                     f"out={_hex_bytes(detail['output'])}")
    except KeyError as exc:
        converted = f"not-codeword(illegal byte 0x{exc.args[0]:02X})"

    pos_order = " ".join(
        f"b{p}->n{nibble_order[p]}" for p in range(4))
    print(f"{prefix} {frame} {rel} cw={cw:<6} field={field:<13} "
          f"in={_hex_bytes(codeword):<11} order=[{pos_order}] "
          f"{converted} note={note}")


def trace_decode(data: bytes,
    ...
    """Print the 4-byte decoding path for frame fields and payload."""
    if trace_end is None:
        trace_end = len(data)

    print("# file_off frame rel cw field input byte/nibble-order "
          "pair-ids nibbles shifts decoded-word output-bytes note")
    print(f"# serialized output byte order: {byte_order}")
    print("# nibble positions are output-word nibbles: "
          "n0=bits15:12 n1=bits11:8 n2=bits7:4 n3=bits3:0")

    if frames and frames[0]["start"] > trace_start:
        leading_start = trace_start
        leading_end = frames[0]["start"]
        for off in range(leading_start, leading_end, 4):
            chunk = bytes(data[off:min(off + 4, leading_end)])
            if len(chunk) == 4:
                _trace_line(off, None, None, "file-header", chunk,
                            mappings, nibble_order, byte_order,
                            "leading bytes before first frame; not decoded")
            else:
                print(f"0x{off:08X} frame=- rel=- cw=-      "
                      f"field=file-header  in={_hex_bytes(chunk):<11} "
                      f"partial={len(chunk)} note=leading partial bytes; "
                      f"not decoded")
    ...
    for f in frames:
        ...
        fields = [
            ("frame-header", start, SYNC_LEN,
             "sync bytes; frame delimiter; stripped from decoded output"),
            ("ffff-marker", start + SYNC_LEN, FFFF_MARKER_LEN,
             "constant FFFF marker codeword; stripped from decoded output"),
            ("metadata", start + SYNC_LEN + FFFF_MARKER_LEN, METADATA_LEN,
             "frame metadata; not in payload stream; stripped"),
        ]
        for field, off, size, note in fields:
            chunk = bytes(data[off:off + size])
            if len(chunk) == 4:
                _trace_line(off, frame_index, off - start, field, chunk,
                            mappings, nibble_order, byte_order, note)

        p_start = f["payload_start"]
        p_size = f["payload_size"]
        used = (max(0, p_size) // 4) * 4
        pbytes = bytes(data[p_start:p_start + used])
        payload_ok = all(
            pbytes[k] in PAIR_INDEX[k % 4] for k in range(used))
        include_payload = (
            p_size > 0 and
            (f["is_standard"] or (not only_standard and payload_ok))
        )

        if p_size > 0:
            for off in range(p_start, p_start + used, 4):
                chunk = bytes(data[off:off + 4])
                note = "payload data codeword"
                cw_idx = output_cw if include_payload else None
                if not include_payload:
                    if only_standard and not f["is_standard"]:
                        note = "anomalous-frame payload skipped by --only-standard"
                    elif not payload_ok:
                        note = "anomalous payload contains non-codeword bytes; skipped"
                    else:
                        note = "payload skipped"
                _trace_line(off, frame_index, off - start, "data", chunk,
                            mappings, nibble_order, byte_order, note, cw_idx)
                if include_payload:
                    output_cw += 1
            if p_size % 4:
                off = p_start + used
                chunk = bytes(data[off:p_start + p_size])
                print(f"0x{off:08X} frame={frame_index:04d} "
                      f"rel=0x{off - start:04X} cw=-      field=data         "
                      f"in={_hex_bytes(chunk):<11} partial={len(chunk)} "
                      f"note=payload tail is not a complete 4-byte codeword")

        tag = bytes(data[end - TAG_LEN:end]) if len(f["tag"]) == TAG_LEN else b""
        if len(tag) == TAG_LEN:
            _trace_line(end - TAG_LEN, frame_index, end - TAG_LEN - start,
                        "tag", tag, mappings, nibble_order, byte_order,
                        "frame tag; bytes 0-1 may carry tag data; "
                        "bytes 2-3 are tag marker; stripped")

    if frames and frames[-1]["end"] < trace_end:
        trailing_start = frames[-1]["end"]
        for off in range(trailing_start, trace_end, 4):
            chunk = bytes(data[off:min(off + 4, trace_end)])
            if len(chunk) == 4:
                _trace_line(off, None, None, "trailer", chunk,
                            mappings, nibble_order, byte_order,
                            "trailing bytes after last frame; not decoded")
            else:
                print(f"0x{off:08X} frame=- rel=- cw=-      field=trailer      "
                      f"in={_hex_bytes(chunk):<11} partial={len(chunk)} "
                      f"note=trailing partial bytes; not decoded")
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--trace-decode", action="store_true",
                    help="print one diagnostic line per 4 input bytes, "
                         "showing frame field, byte/nibble order, pair ids, "
                         "decoded word, output bytes, and notes")
    ...
    if args.trace_decode:
        if not anchors:
            print("[!] no anchors supplied; trace uses heuristic mapping, "
                  "so decoded words/output bytes are probably wrong",
                  file=sys.stderr)
        mappings = derive_mappings(pair_rows, anchors, nibble_order)
        trace_decode(data, frames, mappings, nibble_order, args.byte_order,
                     only_standard=args.only_standard,
                     trace_start=args.skip,
                     trace_end=args.end if args.end is not None else len(data))
        return 0
