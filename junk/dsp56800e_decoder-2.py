...
File format (from analysis):
    Stream of frames separated by 4-byte sync + 4-byte tag.  Frames are
    almost always 168 bytes (= 4 sync + 4 tag + 40 payload codewords =
    40 16-bit DSP words).  A few frames are anomalous: the first frame
    at offset 0 is 40 bytes (header), one mid-file frame is 232 bytes
    (likely a section header), and the last frame is 108 bytes
    (truncated tail).
...
Decoding pipeline:
    1. find_frames        — locate every sync + validate tag suffix.
    2. strip_framing      — concatenate post-framing bytes from frames.
...
def find_frames(data: bytes,
    ...
    A frame begins at any byte offset whose first 4 bytes match a valid
    sync and whose next 4 bytes have a valid tag suffix
    (bytes 2-3 ∈ {(0x26,0x9F),(0xBF,0x53)}).  Searching is byte-by-byte
    rather than 4-aligned so that files with a non-aligned leading
    header (e.g. an ELF or proprietary wrapper) are handled cleanly.
    ...
    Returns:
        Frames in file order; each frame's `frame_size` is the distance
        to the next sync (or to `end_offset` for the final frame).
    """
    if end_offset is None:
        end_offset = len(data)
    last_search = end_offset - FRAMING + 1

    sync_positions: list[int] = []
    pos = start_offset
    while pos < last_search:
        if bytes(data[pos:pos + SYNC_LEN]) in VALID_SYNCS:
            b2 = data[pos + SYNC_LEN + 2]
            b3 = data[pos + SYNC_LEN + 3]
            if (b2, b3) in VALID_TAG_SUFFIXES:
                sync_positions.append(pos)
                # Frames are at least FRAMING long; skip ahead to avoid
                # matching inside a payload by chance.
                pos += FRAMING
                continue
        pos += 1
    ...
    if not sync_positions:
        raise ValueError(
            "no valid sync words found in file; "
            "is this the right format? "
            "(expected sync ∈ {99 56 87 68, 99 55 83 68, 99 56 83 68} "
            "followed by tag with bytes 2-3 ∈ {(26,9F),(BF,53)})")
    ...
    for i, start in enumerate(sync_positions):
        ...
        tag = bytes(data[start + SYNC_LEN:start + FRAMING])
        frames.append({
            "index": i,
            "start": start,
            "end": end,
            ...
            "sync": sync,
            "tag": tag,
            "payload_start": start + FRAMING,
            "payload_size": size - FRAMING,
    ...
    for f in frames:
        ...
        if not f["is_standard"]:
            anomalous_blocks.append({
                ...
                "sync": f["sync"].hex(),
                "tag": f["tag"].hex(),
                "payload_raw": bytes(data[p_start:f["end"]]),
            })
            if only_standard:
                continue
...
def _info(data: bytes,
    ...
    print(f"  standard         : {n_std}", file=sys.stderr)
    ...
    if anomalous:
        print("anomalous frames:", file=sys.stderr)
        for a in anomalous:
            print(f"  frame {a['frame_index']:>4} : {a['frame_size']:>4} bytes  "
