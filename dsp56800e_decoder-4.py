...
def main(argv=None) -> int:
    ...
    if args.find_runs is not None:
        ...
        for i, r in enumerate(runs):
            ...
            # Show the actual byte quadruple at the run start (first
            # polarity choice — both polarities of each pair-id share
            # the same logical nibble, so any byte position will do).
            byte_offset = frames[0]['payload_start']  # rough
            pid_str = "(%X,%X,%X,%X)" % cw
